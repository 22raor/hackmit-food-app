...
def extract_json_from_html(html_content):
    ...
    matches = re.search(
        r'self\.__next_f\.push\(\[1,\s*"2f:(.*)\]\)', html_content, re.DOTALL
    )
    ...
    try:
        ...
        data = data["platformProps"]["additionalPlatformProps"]["apolloCacheData"][0][
            "data"
        ]["storepageFeed"]
        return True, data
...
def json_extract(data):
    ...
    restaurant_info = {
        ...
        "review_count": get_nested_val(
            store_header, ["ratings", "numRatingsDisplayString"]
        ),
        ...
        "other_info": {
            ...
            "delivery_time": get_nested_val(
                store_header, ["deliveryTimeLayout", "title"], "N/A"
            ),
            "pickup_time": get_nested_val(
                store_header, ["pickupTimeLayout", "title"], "N/A"
            ),
        },
        "menu_items": [],
    }
    ...
    new_menu_items = []
    for i, item in enumerate(restaurant_info["menu_items"]):
        if item["category"] != "Most Ordered":
            item["item_id"] = i + 1
            new_menu_items.append(item)

    restaurant_info["menu_items"] = new_menu_items
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        if success:
            ...
            status, place_id, reviews = enrich_with_maps(
                data["address"], data["latitude"], data["longitude"], API_KEY
            )
            if status:
