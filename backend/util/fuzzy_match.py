    ...
    def match_menu_items(
        ...
        for item in menu_items:
            ...
            name_score = FuzzyMatcher._calculate_similarity(
                user_input_clean, FuzzyMatcher._clean_string(name)
            )

            desc_score = FuzzyMatcher._calculate_similarity(
                user_input_clean, FuzzyMatcher._clean_string(description)
            )
            ...
            partial_score = FuzzyMatcher._check_partial_matches(
                user_input_clean, name, description
            )
...
class RestaurantMatcher:
    ...
    def match_cuisine_types(
        user_preference: str, available_cuisines: List[str], threshold: float = 0.7
    ) -> List[Tuple[str, float]]:
        """Match user cuisine preference to available cuisine types"""
        return FuzzyMatcher.find_all_matches(
            user_preference, available_cuisines, threshold
        )
        ...
        for item in menu_items:
            ...
            for allergen in allergens:
                if (
                    FuzzyMatcher._calculate_similarity(
                        restriction_clean, FuzzyMatcher._clean_string(allergen)
                    )
                    > 0.8
                ):
                    is_safe = False
            ...
            if is_safe:
                for ingredient in ingredients:
                    if (
                        FuzzyMatcher._calculate_similarity(
                            restriction_clean, FuzzyMatcher._clean_string(ingredient)
                        )
                        > 0.8
                    ):
                        is_safe = False
