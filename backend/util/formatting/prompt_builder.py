...
class PromptBuilder:
    ...
    def _build_user_profile_section(user_profile: Dict[str, Any]) -> str:
        ...
        if restrictions:
            restriction_names = [r.get('name', '') for r in restrictions if isinstance(r, dict)]
        ...
        if liked_foods:
            food_names = [f.get('name', f) if isinstance(f, dict) else str(f) for f in liked_foods]
            sections.append(f"- Previously liked: {', '.join(food_names[:5])}")
        
        disliked_foods = user_profile.get('disliked_foods', [])
        if disliked_foods:
            food_names = [f.get('name', f) if isinstance(f, dict) else str(f) for f in disliked_foods]
